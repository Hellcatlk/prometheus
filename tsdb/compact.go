...
type blockChunkSeriesSet struct {
	...
	current   *blockChunkSeries
	intervals tombstones.Intervals

	err error
}

func newBlockChunkSeriesSet(i IndexReader, c ChunkReader, t tombstones.Reader, p index.Postings, mint, maxt int64) *blockChunkSeriesSet {
...
func (b *blockChunkSeriesSet) Next() bool {
	for b.p.Next() {
		...
		if err := b.index.Series(b.p.At(), &lbls, &chks); err != nil {
			b.err = errors.Wrapf(err, "get series %d", b.p.At())
			break
		}

		b.current = &blockChunkSeries{
			labels: lbls,
			iter: &populateWithTombstonesChunkIterator{
				chunks:     b.chunks,
				mint:       b.mint,
				maxt:       b.maxt,
				chks:       chks,
...
type blockChunkSeries struct {
	iter   chunks.Iterator
	labels labels.Labels
}

func (b *blockChunkSeries) Labels() labels.Labels {
	return b.labels
}

func (b *blockChunkSeries) Iterator() chunks.Iterator {
	return b.iter
}
...
type populateWithTombstonesChunkIterator struct {
	...
	// Requested min and max time range to trim chunks for.
	mint, maxt int64
...
func (p *populateWithTombstonesChunkIterator) Next() bool {
	for p.err == nil && p.i < len(p.chks)-1 {
		...
		// * Requested time range are closed: [req.Start, req.End].

		if p.curr.MaxTime < p.mint {
			// Fully before the requested range.
			continue
		}

		if p.curr.MinTime > p.maxt {
		...
		if p.curr.MaxTime > p.maxt {
			p.bufDelIter.intervals = append(p.bufDelIter.intervals, tombstones.Interval{Mint: p.maxt + 1, Maxt: math.MaxInt64})
		}

		if p.curr.MinTime < p.mint {
			p.bufDelIter.intervals = append(p.bufDelIter.intervals, tombstones.Interval{Mint: math.MinInt64, Maxt: p.mint - 1})
		}
		...
		// TODO(bwplotka): Do not when Samples requested!
		// Re-encode the chunk to not have deleted values or if they are still open from case above.
		if len(p.bufDelIter.intervals) > len(p.intervals) ||
			(len(p.intervals) > 0 && p.curr.OverlapsClosedInterval(p.bufDelIter.intervals[0].Mint, p.bufDelIter.intervals[len(p.bufDelIter.intervals)-1].Maxt)) {
